...
async function boot() {
  ...
  Object.keys(meta.categories).forEach((key) => {
    const c = meta.categories[key];
    const a = document.createElement("a");      // real link → its own URL (/uav/, /sensors/)
    a.className = "tab"; a.href = "/" + key + "/"; a.dataset.key = key;
    a.innerHTML = `${c.label}<span class="n">${c.count}</span>`;
    a.addEventListener("click", (e) => { e.preventDefault(); go(key); });
    els.tabs.appendChild(a);
  });
  ...
  window.addEventListener("popstate", () => applySection(sectionFromPath()));
  applySection(sectionFromPath());
}
// Which top-level section (UAVs / Sensors / future chapters) the URL points to.
function sectionFromPath() {
  const m = location.pathname.match(/^\/([^/]+)\/?/);
  const key = m && m[1];
  return (key && state.meta.categories[key]) ? key : Object.keys(state.meta.categories)[0];
}
function go(key) {
  if (key === state.cat) return;
  history.pushState(null, "", "/" + key + "/");   // update the URL without a reload
  applySection(key);
}
function applySection(key) {
  if (!state.meta.categories[key]) key = Object.keys(state.meta.categories)[0];
  [...els.tabs.children].forEach((a) => a.classList.toggle("active", a.dataset.key === key));
  selectCat(key, state.meta.categories[key].file);
}
async function selectCat(key, file) {
  state.cat = key;
